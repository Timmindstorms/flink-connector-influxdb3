 ...
 * - Updated package to dev.timmindstorms.flink.streaming.connectors.influxdb3.sink.writer.
 */
package dev.timmindstorms.flink.streaming.connectors.influxdb3.sink.writer;
...
import static dev.timmindstorms.flink.streaming.connectors.influxdb3.sink.InfluxDBSinkOptions.*;

/**
 * This Class implements the {@link SinkWriter} and it is responsible to write
 * incoming inputs to
 * InfluxDB. It uses the {@link InfluxDBSchemaSerializer} to serialize the input
 * into a {@link
 * Point} object. Each serialized object is stored in the {@link #elements}
 * list. Whenever the size
 * of the list reaches the {@link #bufferSize}, the influxDB write API is called
 * and all the items
 * all written. The {@link #lastTimestamp} keeps track of the biggest timestamp
 * of the incoming
 * elements.
 *
...
public final class InfluxDBWriter<IN> implements SinkWriter<IN, Long, Point> {
    ...
    /**
     * This method is called whenever a checkpoint is set by Flink. It creates a
     * list and fills it
    ...
    private void writeCurrentElements() {
        this.influxDBClient.writePoints(this.elements);
